    ...
    <div className="fixed inset-0 w-full h-full bg-[#050505] overflow-hidden flex flex-col select-none touch-none font-sans">
      ...
      <div className="w-full p-3 md:p-4 bg-zinc-950 border-b border-zinc-800 flex justify-between items-center z-[200] shadow-2xl shrink-0">
        <div className="flex gap-4 items-center">
          ...
          <button onClick={() => setShowSidebar(!showSidebar)} className="px-4 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-[9px] font-black uppercase text-white hover:border-indigo-500 transition-all">Personagens</button>
      ...
      <div className="flex-1 w-full flex overflow-hidden relative">
        ...
        <div className={`${showSidebar ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 absolute md:relative z-[150] w-72 h-full bg-zinc-950 border-r border-zinc-800 p-6 overflow-y-auto flex flex-col shadow-2xl shrink-0`}>
          <h3 className="text-[10px] font-black uppercase text-indigo-500 tracking-widest mb-6 italic">Agentes em Campo</h3>
          ...
          <button onClick={spawnNPC} className="w-full p-4 bg-red-950/10 border border-red-900/20 rounded-xl text-[10px] font-black uppercase text-red-500 hover:bg-red-900/20 transition-all shadow-lg">+ Adicionar NPC</button>
        </div>
        ...
        <div className="relative flex-1 bg-black overflow-hidden">
          ...
          >
            <TransformComponent wrapperClass="!w-full !h-full" contentClass="!w-full !h-full">
              <div className="relative w-[2000px] h-[2000px] flex items-center justify-center" onClick={() => setSelectedToken(null)}>
                ...
                {activeTokens.map((t) => {
                  ...
                  return (
                    <Draggable 
                      ...
                      onStart={() => setSelectedToken({ id: t.id, type: t.type })}
                    >
                      <div className="absolute z-40 group cursor-grab active:cursor-grabbing" onContextMenu={(e) => rotacionarToken(e, t.id)} onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="relative z-[40]">
                          <img 
                            src={(data as any).foto || (t.type === 'player' ? 'https://i.pinimg.com/originals/ae/2e/56/ae2e562090e0ea66904128f898236113.png' : 'https://i.imgur.com/ae2e562.png')} 
                            className={`w-32 h-32 md:w-40 md:h-40 object-contain pointer-events-none transition-all duration-300 ${isSelected ? 'brightness-125' : ''}`}
                            style={{ transform: `rotate(${t.rotacao}deg)` }}
                          />
                        </div>
                      </div>
                    </Draggable>
        ...
        {selectedToken && selectedData && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 md:translate-x-0 md:left-auto md:right-8 md:top-24 z-[300] bg-zinc-950/95 border border-zinc-800 p-6 md:p-8 rounded-[2rem] w-[92%] md:w-80 shadow-[0_0_100px_rgba(0,0,0,0.8)] backdrop-blur-3xl animate-in slide-in-from-bottom-10 md:slide-in-from-right-10" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setSelectedToken(null)} className="text-zinc-600 hover:text-white transition-colors text-3xl">✕</button>
            </div>
            ...
            <div className="space-y-8">
              <div className="flex items-center justify-between p-5 bg-zinc-900/50 border border-zinc-800 rounded-3xl shadow-inner">
                ...
                <button 
                  onClick={(e) => { e.stopPropagation(); toggleLanterna(selectedToken.id); }} 
                  className={`w-16 h-8 rounded-full transition-all relative ${currentToken?.lanternaAtiva ? 'bg-indigo-600 shadow-[0_0_20px_rgba(79,70,229,0.5)]' : 'bg-zinc-800'}`}
                >
              ...
              <div>
                ...
                <input 
                  ...
                  onChange={(e) => handleUpdateResource(selectedToken.id, selectedToken.type, 'vidaAtual', Number(e.target.value))} 
                  className="w-full h-2 bg-zinc-900 rounded-lg appearance-none cursor-pointer accent-red-600 shadow-inner" 
                />
              </div>
              ...
              {selectedToken.type === 'player' && (
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => handleUpdateResource(selectedToken.id, selectedToken.type, 'sanidadeAtual', Number(e.target.value))} 
                    className="w-full h-2 bg-zinc-900 rounded-lg appearance-none cursor-pointer accent-cyan-600 shadow-inner" 
                  />
                </div>
              )}
              
              <button 
                onClick={() => { setActiveTokens(prev => prev.filter(t => t.id !== selectedToken.id)); setSelectedToken(null); }} 
